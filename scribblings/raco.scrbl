...
@title[#:tag "raco-beeswax"]{@exec{raco beeswax render}}

This command renders a Pollen source (or any file that provides Pollen’s @seclink["Standard_exports"
#:doc '(lib "pollen/scribblings/pollen.scrbl")]{standard identifiers} @racket[_doc] and
@racket[_metas]) using the appropriate Beeswax template.
...
@itemlist[
 ...
 @item{The optional @exec{-t} or @exec{--target} switch specifies the target file type to use for
...
@racket[(setup:poly-targets)]. }

 @item{This command makes use of @seclink["Cache" #:doc '(lib
"pollen/scribblings/pollen.scrbl")]{Pollen’s @emph{compile} cache}; if the source file has not been
compiled, it will be compiled and cached; if it has already been compiled, the cached copy is used.}
...
@item{You can only specify @emph{source files} --- not directories and not output files. If you want
to render a bunch of files, list them out separately or use wildcards. In any case, each output
filename is derived from the source filename, incorporating the @exec{--target} value to get the
extension when necessary. }

@item{You cannot specify @secref["Preprocessor___pp_extension_" #:doc '(lib
"pollen/scribblings/pollen.scrbl")] files or @seclink["top" #:doc'(lib
"scribblings/scribble/scribble.scrbl")]{Scribble files} with this command.}

@item{There is not (yet) a parallel rendering option. All the files are rendered in sequence using a
...
full use of all available cores.}

@item{This command does not use Pollen’s (or any) @emph{render} caching; it will always write the
...
Perhaps Beeswax will do this for you at some point, but for now, it doesn’t. You can either do it
...
Here we compare three render times for the same source document: one using Pollen (with a Pollen
template), one using @exec{raco beeswax render}, and another using @exec{raco beeswax render} with a
